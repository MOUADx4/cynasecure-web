...
class CheckoutController extends AbstractController
{
    ...
    #[OA\RequestBody(content: new OA\JsonContent(properties: [
        new OA\Property(property: 'items', type: 'array', items: new OA\Items(properties: [
            new OA\Property(property: 'serviceId', type: 'integer'),
            new OA\Property(property: 'billing', type: 'string', enum: ['monthly', 'yearly']),
        ])),
        new OA\Property(property: 'address', type: 'object'),
        new OA\Property(property: 'promoCode', type: 'string'),
        new OA\Property(property: 'guestEmail', type: 'string', format: 'email'),
    ]))]
    #[OA\Response(response: 200, description: 'Intent de paiement créé')]
    ...
    public function intent(
        ...
        $paypalReturnUrl = isset($data['paypalReturnUrl']) ? trim((string) $data['paypalReturnUrl']) : null;
        ...
        try {
            $ppCancelUrl = $paypalReturnUrl ? str_replace('/success', '/cancel', $paypalReturnUrl) : null;
            $ppOrder     = $paypal->createOrder($total, $paypalItems, $paypalReturnUrl, $ppCancelUrl);
            $order->setPaypalOrderId($ppOrder['id'] ?? null);

            foreach ($ppOrder['links'] ?? [] as $link) {
                if (in_array($link['rel'] ?? '', ['approve', 'payer-action'], true)) {
                    $paypalApproveUrl = $link['href'];
                    break;
                }
            }
        } catch (\Exception $e) {
        ...
        return new JsonResponse([
            ...
            'paypalApproveUrl' => $paypalApproveUrl,
            'orderId'         => $order->getId(),
            'total'           => $total,
            'discount'        => $discount,
    ...
    #[OA\RequestBody(content: new OA\JsonContent(properties: [
        ...
        new OA\Property(property: 'gateway', type: 'string', enum: ['stripe', 'paypal']),
        new OA\Property(property: 'paymentIntentId', type: 'string'),
        new OA\Property(property: 'paypalOrderId', type: 'string'),
    ]))]
    ...
    #[OA\Response(response: 200, description: 'Facture PDF')]
